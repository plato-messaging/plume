...
final boldStyle = Style().merge(Attribute.bold);
final h1Style = Style().merge(Attribute.h1);
final h2Style = Style().merge(Attribute.h2);
final ulStyle = Style().merge(Attribute.ul);
final bqStyle = Style().merge(Attribute.bq);
final rightStyle = Style().merge(Attribute.right);
...
void main() {
  group('$LineNode', () {
    ...
    test('toString', () {
      ...
      node.applyAttribute(Attribute.h1);
    ...
    test('format line', () {
      ...
      root.retain(27, 1, Style().merge(Attribute.cl));
      ...
      final delta = Delta()
        ..insert('Hello world')
        ..insert('\n', {
          Attribute.h1.key: Attribute.h1.value,
          Attribute.alignment.key: Attribute.right.value,
        });
      expect(line.toDelta(), delta);
    });
    ...
    test('format root line to unset block style', () {
      final unsetBlock = Style().put(Attribute.block.unset);
      root.insert(0, 'Hello world', null);
      root.retain(11, 1, unsetBlock);
      expect(root.childCount, 1);
      expect(root.first, const TypeMatcher<LineNode>());
      final line = root.first as LineNode;
      expect(line.style.contains(Attribute.block), isFalse);
    });
    ...
    test('delete line-break of non-empty line', () {
      ...
      expect(line.style.get(Attribute.heading), Attribute.h2);
    });
