...
class PlumeHistory extends StatefulWidget {
  /// Creates an instance of [PlumeHistory].
  const PlumeHistory({
    super.key,
    required this.child,
  ...
  /// The child widget of [PlumeHistory].
  final Widget child;

  /// The [PlumeController] to save the state of over time.
  final PlumeController controller;

  @override
  State<PlumeHistory> createState() => _PlumeHistoryState();
}

class _PlumeHistoryState extends State<PlumeHistory> {
  ...
  @override
  void didUpdateWidget(PlumeHistory oldWidget) {
...
class HistoryStack {
  ...
  HistoryStack.doc(Document? doc)
    : this(doc?.toDelta() ?? Document().toDelta());
