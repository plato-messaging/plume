...
class AutoFormats {
  ...
  bool run(Document document, int position, Object data) {
  ...
  TextSelection? undoActive(Document document) {
...
class AutoFormatLinks extends AutoFormat {
  ...
  AutoFormatResult? apply(Document document, int position, String data) {
    ...
    if (attributes.containsKey(Attribute.link.key)) return null;
...
class MarkdownInlineShortcuts extends AutoFormat {
  static final rules = <String, Attribute>{
    '**': Attribute.bold,
    '*': Attribute.italic,
  ...
  AutoFormatResult? apply(Document document, int position, String data) {
    ...
    for (final String rule in rules.keys) {
      if (candidate.endsWith(rule)) {
        ...
        final startOffset = candidate
            .substring(0, lastOffset)
            .lastIndexOf(rule);
...
class MarkdownLineShortcuts extends AutoFormat {
  static final rules = <String, Attribute>{
    '-': Attribute.block.bulletList,
    '*': Attribute.block.bulletList,
    '1.': Attribute.block.numberList,
    '[]': Attribute.block.checkList,
    "'''": Attribute.block.code,
    '```': Attribute.block.code,
    '>': Attribute.block.quote,
    '#': Attribute.h1,
    '##': Attribute.h2,
    '###': Attribute.h3,
  };
    ...
    if (data == '`' || data == "'") {
      ...
      if (shortcut == '```' || shortcut == "'''") {
        ...
        return AutoFormatResult(
          selection: result.$1,
          change: change,
          undoSelection: TextSelection.collapsed(
            offset: position + data.length,
          ),
          undo: undo,
...
class AutoTextDirection extends AutoFormat {
  ...
  AutoFormatResult? apply(Document document, int position, String data) {
    ...
    if (_isRTL(data)) {
      attributes = {
        ...Attribute.rtl.toJson(),
        ...Attribute.alignment.right.toJson(),
      };
    } else {
      attributes = {
        ...Attribute.rtl.unset.toJson(),
        ...Attribute.alignment.unset.toJson(),
      };
    }
