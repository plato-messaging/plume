...
Widget defaultSpellCheckMenuBuilder(
  ...
  switch (defaultTargetPlatform) {
    case TargetPlatform.iOS:
      return PlumeCupertinoSpellCheckSuggestionsToolbar.editor(
        editorState: editorState,
      );
...
class PlumeSpanEmbedConfiguration {
  const PlumeSpanEmbedConfiguration(
    this.embedBuilder, {
...
Widget defaultPlumeEmbedBuilder(BuildContext context, EmbedNode node) {
  if (node.value.type == 'hr') {
    final plumeThemeData = PlumeTheme.of(context)!;

    return Divider(
      height: plumeThemeData.horizontalRule.height,
      thickness: plumeThemeData.horizontalRule.thickness,
      color: plumeThemeData.horizontalRule.color,
    );
  }
  throw UnimplementedError(
    'Embeddable type "${node.value.type}" is not supported by default embed '
    'builder of PlumeEditor. You must pass your own builder function to '
    'embedBuilder property of PlumeEditor or PlumeField widgets.',
  );
}

/// Widget for editing rich text documents.
class PlumeEditor extends StatefulWidget {
  ...
  final PlumeContextMenuBuilder contextMenuBuilder;
  ...
  ///
  /// Plume provides default implementation which can be overridden by this
  ...
  @override
  State<PlumeEditor> createState() => _PlumeEditorState();
}

class _PlumeEditorState extends State<PlumeEditor>
  ...
  @override
  void didUpdateWidget(covariant PlumeEditor oldWidget) {
  ...
  void initState() {
    ...
    _selectionGestureDetectorBuilder =
        _PlumeEditorSelectionGestureDetectorBuilder(state: this);
  }
  ...
  Widget build(BuildContext context) {
    ...
    child = PlumeShortcuts(
      child: PlumeActions(
        child: PlumeHistory(controller: widget.controller, child: child),
      ),
    );
...
class _PlumeEditorSelectionGestureDetectorBuilder
    extends EditorTextSelectionGestureDetectorBuilder {
  _PlumeEditorSelectionGestureDetectorBuilder({
    required _PlumeEditorState state,
  }) : _state = state,
       super(delegate: state);

  final _PlumeEditorState _state;
...
class RawEditor extends StatefulWidget {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<PlumeController>('controller', controller),
    );
...
class RawEditorState extends EditorState
  ...
  // Theme
  late PlumeThemeData _themeData;

  @override
  PlumeThemeData get themeData => _themeData;

  // Cursors
  late CursorController _cursorController;

  PlumeController get controller => widget.controller;
  ...
  void _setClipboardData() {
    final TextSelection selection = textEditingValue.selection;
    widget.clipboardManager.setData(
      PlumeClipboardData(
  ...
  VerticalSpacing _getSpacingForLine(LineNode node, PlumeThemeData theme) {
    final style = node.style.get(Attribute.heading);
    if (style == Attribute.heading.level1) {
      return theme.heading1.spacing;
    } else if (style == Attribute.heading.level2) {
      return theme.heading2.spacing;
    } else if (style == Attribute.heading.level3) {
      return theme.heading3.spacing;
    } else if (style == Attribute.heading.level4) {
      return theme.heading4.spacing;
    } else if (style == Attribute.heading.level5) {
  ...
  VerticalSpacing _getSpacingForBlock(BlockNode node, PlumeThemeData theme) {
    final style = node.style.get(Attribute.block);
    if (style == Attribute.block.code) {
      return theme.code.spacing;
    } else if (style == Attribute.block.quote) {
  ...
  _TextBoundary _characterBoundary(DirectionalTextEditingIntent intent) {
    final _TextBoundary atomicTextBoundary = _CharacterBoundary(
      textEditingValue,
    );
  ...
  late final Map<Type, Action<Intent>> _actions = <Type, Action<Intent>>{
    ...
    ExpandSelectionToLineBreakIntent: _makeOverridable(
      _UpdateTextSelectionAction(this, false, _linebreak),
    ),
  ...
  TextSelectionToolbarAnchors get contextMenuAnchors {
    ...
    final List<TextSelectionPoint> endpoints = renderEditor
...
>
  ...
  PlumeVerticalCaretMovementRun? _verticalMovementRun;
    ...
    TextPosition computeNewExtent() {
      ...
      if (intent.forward) {
        if (collapseSelection) {
          state.updateLastKnownWithSelection(
            TextSelection.collapsed(offset: state.textEditingValue.text.length),
          );
        }
