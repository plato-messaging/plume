...
///
/// Descendant widgets obtain the current theme's [PlumeThemeData] object using
/// [PlumeTheme.of].
///
/// See also:
///
///   * [PlumeThemeData], which describes actual configuration of a theme.
class PlumeTheme extends InheritedWidget {
  final PlumeThemeData data;

  /// Applies the given theme [data] to [child].
  ///
  /// The [data] and [child] arguments must not be null.
  const PlumeTheme({super.key, required this.data, required super.child});

  @override
  bool updateShouldNotify(PlumeTheme oldWidget) {
    return data != oldWidget.data;
  }

  /// The data from the closest [PlumeTheme] instance that encloses the given
  /// context.
  ///
  /// Returns `null` if there is no [PlumeTheme] in the given build context
  /// and [nullOk] is set to `true`. If [nullOk] is set to `false` (default)
  /// then this method asserts.
  static PlumeThemeData? of(BuildContext context, {bool nullOk = false}) {
    final widget = context.dependOnInheritedWidgetOfExactType<PlumeTheme>();
    if (widget == null && nullOk) return null;
    assert(
      widget != null,
      '$PlumeTheme.of() called with a context that does not contain a PlumeTheme.',
    );
    return widget!.data;
  }
}
...
class VerticalSpacing {
  ...
  const VerticalSpacing.zero() : top = 0.0, bottom = 0.0;
}

class PlumeThemeData {
  /// Style of bold text.
  ...
  factory PlumeThemeData.fallback(BuildContext context) {
    final themeData = Theme.of(context);
    final defaultStyle = DefaultTextStyle.of(context);
    final baseStyle = defaultStyle.style.copyWith(fontSize: 16.0, height: 1.3);
  ...
  PlumeThemeData merge(PlumeThemeData other) {
    return copyWith(
      bold: other.bold,
      italic: other.italic,
...
class InlineCodeThemeData {
  ...
  TextStyle styleFor(Style lineStyle) {
    if (lineStyle.containsSame(Attribute.h1)) {
      return heading1 ?? style;
    } else if (lineStyle.containsSame(Attribute.h2)) {
      return heading2 ?? style;
    } else if (lineStyle.containsSame(Attribute.h3)) {
