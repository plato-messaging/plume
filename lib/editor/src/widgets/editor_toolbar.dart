...
class UndoRedoButton extends StatelessWidget {
  ...
  const UndoRedoButton.undo({Key? key, required PlumeController controller})
    : this._(controller, _UndoRedoButtonVariant.undo, key: key);

  const UndoRedoButton.redo({Key? key, required PlumeController controller})
...
class LinkStyleButton extends StatefulWidget {
  ...
  const LinkStyleButton({super.key, required this.controller, this.icon});
...
class ToggleStyleButton extends StatefulWidget {
  ...
  /// Controller attached to a Plume editor.
  final PlumeController controller;
...
class ColorButton extends StatefulWidget {
  ...
  final PlumeController controller;
  final ColorAttributeBuilder attributeKey;
...
class _ColorButtonState extends State<ColorButton> {
  ...
  void _didChangeEditingValue() {
    setState(() {
      ...
      _value = selectionColor?.value != null
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: RawMaterialButton(
        ...
        child: color == null
            ? Text(nullColorLabel, style: Theme.of(context).textTheme.bodySmall)
            : null,
      ),
    );
  }
...
Map<Attribute<int>, String> _headingToText(BuildContext context) {
  final localizations = context.l;

  return {
    Attribute.heading.unset: localizations.headingNormal,
    Attribute.heading.level1: localizations.headingLevel1,
    Attribute.heading.level2: localizations.headingLevel2,
    Attribute.heading.level3: localizations.headingLevel3,
    Attribute.heading.level4: localizations.headingLevel4,
    Attribute.heading.level5: localizations.headingLevel5,
    Attribute.heading.level6: localizations.headingLevel6,
  };
}
...
class _SelectHeadingButtonState extends State<SelectHeadingButton> {
  ...
  Style get selectionStyle => widget.controller.getSelectionStyle();
  ...
  Future<Attribute<int>?> _selectHeading() async {
    final themeData = PlumeTheme.of(context)!;

    final completer = Completer<Attribute<int>?>();
...
class _HeadingList extends StatelessWidget {
  ...
  final PlumeThemeData theme;
  final void Function(Attribute<int>) onSelected;
    ...
    return SingleChildScrollView(
      child: ConstrainedBox(
        ...
        child: Column(
          ...
          children: _headingToText(context).entries
  ...
  Widget _listItem(
    ...
    final valueToStyle = {
      ...
      Attribute.heading.level2: theme?.heading2.style,
      Attribute.heading.level3: theme?.heading3.style,
      Attribute.heading.level4: theme?.heading4.style,
      Attribute.heading.level5: theme?.heading5.style,
      Attribute.heading.level6: theme?.heading6.style,
    };
  ...
  Widget build(BuildContext context) {
    ...
    return FLIconButton(
      ...
      onPressed: isEnabled
          ? () {
              final indentLevel =
                  _selectionStyle.get(Attribute.indent)?.value ?? 0;
              ...
              if (indentLevel == 1 && !widget.increase) {
                ...
                widget.controller.formatSelection(
                  Attribute.indent.withLevel(
                    indentLevel + (widget.increase ? 1 : -1),
                  ),
                );
              }
              PlumeToolbar._of(context).requestKeyboard();
...
class PlumeToolbar extends StatefulWidget implements PreferredSizeWidget {
  ...
  factory PlumeToolbar.basic({
    Key? key,
    required PlumeController controller,
    ...
    return PlumeToolbar(
      ...
      children: [
        ...
        Visibility(
          ...
          child: IndentationButton(increase: false, controller: controller),
        ),
        Visibility(
          visible: !hideIndentation,
          child: IndentationButton(controller: controller),
        ),
        Visibility(
          visible: !hideIndentation,
          child: VerticalDivider(
        ...
        Visibility(
          ...
          child: ToggleStyleButton(
            attribute: Attribute.block.numberList,
            controller: controller,
        ...
        Visibility(
          ...
          child: ToggleStyleButton(
            attribute: Attribute.block.bulletList,
            controller: controller,
        ...
        Visibility(
          ...
          child: ToggleStyleButton(
            attribute: Attribute.block.checkList,
            controller: controller,
        ...
        Visibility(
          ...
          child: ToggleStyleButton(
            attribute: Attribute.block.code,
            controller: controller,
            icon: Icons.code,
          ),
        ),
        ...
        Visibility(
          visible: !hideQuote,
          child: ToggleStyleButton(
            attribute: Attribute.block.quote,
            controller: controller,
        ...
        Visibility(
          ...
          child: UndoRedoButton.undo(controller: controller),
        ),
        Visibility(
          visible: !hideUndoRedo,
          child: UndoRedoButton.redo(controller: controller),
        ),
  ...
  static _PlumeToolbarState _of(BuildContext context) =>
      context.findAncestorStateOfType<_PlumeToolbarState>()!;

  @override
  State<PlumeToolbar> createState() => _PlumeToolbarState();

  @override
  Size get preferredSize => const Size.fromHeight(kToolbarHeight);
}

class _PlumeToolbarState extends State<PlumeToolbar> {
  late PlumeThemeData theme;

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    final parentTheme = PlumeTheme.of(context, nullOk: true);
    final fallbackTheme = PlumeThemeData.fallback(context);
...
/// Default icon button used in Plume editor toolbar.
///
