  ...
  Delta? apply(Delta document, int index, Object data) {
    ...
    if (targetText.startsWith('\n')) {
      if (target.attributes != null &&
          target.attributes!.containsKey(Attribute.heading.key)) {
        // Reset heading style
        resetStyle = Attribute.heading.unset.toJson();
      }
      return Delta()
        ..retain(index)
        ..insert('\n', target.attributes)
...
class AutoExitBlockRule extends InsertRule {
  ...
  Delta? apply(Delta document, int index, Object data) {
    ...
    final blockStyle = target.attributes![Attribute.block.key];
    ...
    if (indent != null && indent > 0) {
      attributes[Attribute.indent.key] = indent - 1;
    } else {
      attributes.addAll(Attribute.block.unset.toJson());
    }
    return Delta()
      ..retain(index)
      ..retain(1, attributes);
...
class PreserveInlineStylesRule extends InsertRule {
  ...
  Delta? apply(Delta document, int index, Object data) {
    ...
    noLinkAttributes.remove(Attribute.link.key);
...
class PreserveBlockStyleOnInsertRule extends InsertRule {
  ...
  Delta? apply(Delta document, int index, Object data) {
    ...
    // original line.
    if (lineStyle.containsKey(Attribute.heading.key)) {
      resetStyle.addAll(Attribute.heading.unset.toJson());
    }

    // Similarly for the checked style
    if (lineStyle.containsKey(Attribute.checked.key)) {
      resetStyle.addAll(Attribute.checked.unset.toJson());
    }
