...
class ResolveLineFormatRule extends FormatRule {
  ...
  Delta _applyAttribute(
    ...
    while (lf >= 0) {
      ...
      Style opStyle = Style.fromJson(op.attributes);
      if (opStyle.containsSame(Attribute.block.checkList) &&
          opStyle.containsSame(Attribute.checked) &&
          attribute.key == Attribute.block.key &&
          attribute.value != Attribute.block.checkList.value) {
        // Unset checked state when changing block style from checkList to something else
        actualStyle[Attribute.checked.key] = null;
      }
      result
        ..retain(lf - offset)
