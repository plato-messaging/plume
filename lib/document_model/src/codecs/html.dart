...
final _inlineAttributesToHtml = {
  Attribute.bold.key: 'strong',
  Attribute.italic.key: 'em',
  Attribute.underline.key: 'u',
  Attribute.strikethrough.key: 'del',
  Attribute.inlineCode.key: 'code',
  Attribute.link.key: 'a',
  Attribute.backgroundColor.key: 'span',
  Attribute.foregroundColor.key: 'span',
};

const _indentWidthInPx = 32;

/// HTML conversion of [Document]
///
...
class HtmlCodec extends Codec<Document, String> {
  const HtmlCodec();
...
// Inline tags relate directly to AttributeScope.inline.
...
//
// Block tags are line attributes that can contain several lines.
// These can be code or lists.
// These behave almost as line tags except there can be nested blocks
class _HtmlEncoder extends Converter<Document, String> {
  const _HtmlEncoder();
  ...
  static bool isPlain(Style style) {
    if (style.isEmpty) return true;
    for (final key in style.keys) {
      if (key == Attribute.alignment.key) continue;
      if (key == Attribute.direction.key) continue;
      if (key == Attribute.indent.key) continue;
      return false;
    }
    return true;
  }
  ...
  static bool isSameBlock(_HtmlBlockTag previous, _HtmlBlockTag current) {
    ...
    if (areOrderedUnorderedLists || areChecklists) {
      final positionAttributes = [
        ...
        Attribute.alignment.right,
        Attribute.alignment.justify,
  ...
  void _handlePlainBlock(Operation op, _EncoderState state) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      _writeData(subOp, buffer);
      _writeTag(buffer, _HtmlLineTag(Style(), position));
...
class _HtmlInlineTag extends _HtmlTag {
  ...
  String get openTag {
    ...
    if (key == Attribute.link.key) {
      return '<${_inlineAttributesToHtml[key]} href="$value">';
    }
    if (key == Attribute.foregroundColor.key) {
      final argb = toRGBA(value);
      return '<${_inlineAttributesToHtml[key]} '
          'style="color: '
          'rgba(${argb.R},${argb.G},${argb.B},${argb.A / 255})">';
    }
    if (key == Attribute.backgroundColor.key) {
      final argb = toRGBA(value);
      return '<${_inlineAttributesToHtml[key]} '
  ...
  @override
  String get closeTag {
    return '</${_inlineAttributesToHtml[attribute.key]}>';
  }
...
// HTML tags that correspond to line attributes in Document
class _HtmlLineTag extends _HtmlTag {
  static bool isLineAttribute(Attribute attribute) {
    return attribute.key == Attribute.heading.key ||
        (attribute.key == Attribute.block.key &&
            (attribute.value == Attribute.code.value ||
                attribute.value == Attribute.ol.value ||
                attribute.value == Attribute.ul.value ||
                attribute.value == Attribute.cl.value ||
                attribute.value == Attribute.bq.value)) ||
        attribute.key == Attribute.alignment.key ||
        attribute.key == Attribute.direction.key ||
        attribute.key == Attribute.checked.key ||
  ...
  bool get isPlain => _HtmlEncoder.isPlain(style);
  ...
  String? get alignmentCss {
    ...
    if (alignment.value == Attribute.alignment.right.value) {
      return '${alignmentPrefix}right$alignmentSuffix';
    }
    if (alignment.value == Attribute.alignment.center.value) {
      return '${alignmentPrefix}center$alignmentSuffix';
    }
    if (alignment.value == Attribute.alignment.justify.value) {
  ...
  String? get blockquoteCss {
    ...
    return style.values.contains(Attribute.bq)
  ...
  String? get indentationCss {
    ...
    if (style.values.contains(Attribute.ul) ||
  ...
  String? get directionAttribute {
    if (_directionAttribute == null) {
      ...
      if (direction.value == Attribute.direction.rtl.value) {
  ...
  String get openTag {
    ...
    for (final attr in style.values) {
      if (attr.key == Attribute.heading.key) {
        openTag += '<h${attr.value}$attribute$css>';
      }
      if (attr.key == Attribute.block.key) {
        if (attr.value == Attribute.bq.value) {
          openTag += '<blockquote$attribute$css>';
        }
        if (attr.value == Attribute.code.value) {
...
// HTMLTags that correspond to block attribute in Document
class _HtmlBlockTag extends _HtmlTag {
  ...
  static bool isBlockAttribute(Attribute attribute) {
    return attribute.key == Attribute.block.key &&
...
class _HtmlDecoder extends Converter<String, Document> {
  const _HtmlDecoder();
  ...
  Style _updateInlineStyle(
    ...
    if (element.localName == 'strong') {
      ...
      updated = updated.put(Attribute.underline);
    } else if (element.localName == 'del') {
      updated = updated.put(Attribute.strikethrough);
  ...
  Style _updateBlockStyle(
    ...
    if (element.localName == 'h1') {
      ...
      updated = updated.put(Attribute.h2);
    } else if (element.localName == 'h3') {
      updated = updated.put(Attribute.h3);
    } else if (element.localName == 'h4') {
      updated = updated.put(Attribute.h4);
    } else if (element.localName == 'h5') {
      updated = updated.put(Attribute.h5);
    } else if (element.localName == 'h6') {
      updated = updated.put(Attribute.h6);
    } else if (element.localName == 'blockquote') {
      updated = updated.put(Attribute.bq);
    } else if (element.localName == 'pre') {
      updated = updated.put(Attribute.code);
    } else if (element.localName == 'code') {
      if (!updated.values.contains(Attribute.code)) {
        updated = updated.put(Attribute.inlineCode);
      }
      ...
      updated = updated.put(Attribute.cl);
      if (element.attributes.containsKey('checked')) {
        updated = updated.put(Attribute.checked);
      }
    }
    ...
    if (dirAttribute != null && dirAttribute == 'rtl') {
      updated = updated.put(Attribute.rtl);
    }
    ...
    for (final style in styles) {
      if (style.startsWith('text-align')) {
        ...
        switch (sValue) {
          case 'right':
            updated = updated.put(Attribute.right);
            continue;
          case 'center':
            updated = updated.put(Attribute.center);
            continue;
          case 'justify':
            updated = updated.put(Attribute.justify);
        }
        continue;
      }
      if (style.startsWith('padding-left')) {
        ...
        updated = updated.put(Attribute.indent.withLevel(1));
        continue;
      }
    }
    return updated;
  ...
  bool _hasList(Style blockStyle) {
    return blockStyle.values.contains(Attribute.ol) ||
        blockStyle.values.contains(Attribute.ul);
  }
}
