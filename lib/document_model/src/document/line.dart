...
/// A line of rich text in a [Document].
///
...
final class LineNode extends ContainerNode<LeafNode> with StyledNode {
  ...
  Style collectStyle(int offset, int length) {
  ...
  void retain(int index, int length, Style? style) {
    ...
    if (isLineFormat) {
      assert(
        style.values.every((attr) => attr.scope == AttributeScope.line),
  ...
  void _formatAndOptimize(Style? newStyle) {
    if (newStyle == null || newStyle.isEmpty) return;

    applyStyle(newStyle);
    if (!newStyle.contains(Attribute.block)) {
      return;
    } // no block-level changes

    final blockStyle = newStyle.get(Attribute.block)!;
    if (parent is BlockNode) {
      final parentStyle = (parent as BlockNode).style.get(Attribute.block);
      if (blockStyle == Attribute.block.unset) {
        unwrap();
    ...
    } else if (blockStyle != Attribute.block.unset) {
