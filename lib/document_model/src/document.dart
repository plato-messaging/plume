...
export 'document/block.dart';
export 'document/leaf.dart';
export 'document/embeds.dart';
export 'document/line.dart';

/// Source of a [DocumentChange].
enum ChangeSource {
...
class DocumentChange {
  DocumentChange(this.before, this.change, this.source);
...
class Document {
  ...
  Document({this._heuristics = Heuristics.fallback})
    : _delta = Delta()..insert('\n') {
    _loadDocument(_delta);
  }

  /// Creates new [Document] from provided JSON `data`.
  Document.fromJson(List data, {this._heuristics = Heuristics.fallback})
    : _delta = _migrateDelta(Delta.fromJson(data)) {
    _loadDocument(_delta);
  }

  /// Creates new [Document] from provided `delta`.
  Document.fromDelta(Delta delta, {this._heuristics = Heuristics.fallback})
    : _delta = _migrateDelta(delta) {
    _loadDocument(_delta);
  }

  final Heuristics _heuristics;
  ...
  /// Stream of [DocumentChange]s applied to this document.
  Stream<DocumentChange> get changes => _controller.stream;
  ...
  /// produces a [DocumentChange] with source set to [ChangeSource.local].
  ///
  /// Returns an instance of [Delta] actually composed into this document.
  ...
  void compose(Delta change, ChangeSource source) {
    ...
    _controller.add(DocumentChange(before, change, source));
  }
  ...
  void _checkMutable() {
    assert(
      ...
      'Cannot modify document after it was closed.',
    );
  }

  /// Key of the embed attribute used in [Document] 0.x (prior to 1.0).
  static const String _kEmbedAttributeKey = 'embed';

  /// Migrates `delta` to the latest format supported by [Document]s.
  ///
  /// Allows backward compatibility with 0.x versions of [Document] package.
