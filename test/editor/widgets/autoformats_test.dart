...
void main() {
  ...
  group('Link detection', () {
    test('Detects link with \'http\' and formats accordingly', () {
      final text = 'Some long text and a https://plume-editor.github.io\n';
      final document = Document.fromJson([
        {'insert': text},
      ]);
      final performed = autoformats.run(document, text.length - 1, ' ');
      ...
      expect(attributes[Attribute.link.key], 'https://plume-editor.github.io');
    });
    ...
    test('Detects link without \'http\' and formats accordingly', () {
      ...
      expect(attributes[Attribute.link.key], 'https://www.github.com');
      expect(autoformats.undoPosition, 35);
    });

    test('No trigger of detection if inserting other than space', () {
      final document = Document.fromJson([
        {'insert': 'Some long text and a https://plume-editor.github.io\n'},
      ]);
    ...
    test('Deleting at candidate position, undoes link formatting', () {
      const text = 'Some long text and a https://plume-editor.github.io\n';
      final document = Document.fromJson([
        {'insert': text},
      ]);
      autoformats.run(document, text.length - 1, ' ');
  ...
  group('Markdown shortcuts', () {
    test('Detects single character shortcuts', () {
      ...
      expect(attributes[Attribute.block.key], Attribute.block.bulletList.value);
    });
    ...
    test('Detects 2 character shortcuts', () {
      ...
      expect(attributes, isNotNull);
      expect(attributes!.containsKey(Attribute.block.key), isTrue);
      expect(attributes[Attribute.block.key], Attribute.block.numberList.value);
    });
    ...
    test('Detects 3 character shortcuts', () {
      ...
      expect(attributes, isNotNull);
      expect(attributes!.containsKey(Attribute.block.key), isTrue);
      expect(attributes[Attribute.block.key], Attribute.block.code.value);
    });
    ...
    test('Detects italic shortcut', () {
      ...
      final attributes = document.toDelta().toList()[1].attributes;
      expect(attributes![Attribute.italic.key], isTrue);
    });
    ...
    test('Detects bold shortcut', () {
      ...
      final attributes = document.toDelta().toList()[1].attributes;
      expect(attributes![Attribute.bold.key], isTrue);
    });
    ...
    test('Detects inline code shortcut', () {
      ...
      expect(autoformats.selection, const TextSelection.collapsed(offset: 20));
      final attributes = document.toDelta().toList()[1].attributes;
      expect(attributes![Attribute.inlineCode.key], isTrue);
      final undoSelection = autoformats.undoActive(document);
    ...
    test('Detects strikethrough shortcut', () {
      ...
      expect(autoformats.selection, const TextSelection.collapsed(offset: 20));
      final attributes = document.toDelta().toList()[1].attributes;
      expect(attributes![Attribute.strikethrough.key], isTrue);
      final undoSelection = autoformats.undoActive(document);
  ...
  group('RTL detection', () {
    test('Detects RTL text when applied with no line style', () {
      ...
      final attributes = document.toDelta().toList()[1].attributes;
      expect(attributes, isNotNull);
      expect(attributes!.containsKey(Attribute.direction.key), isTrue);
      expect(
        attributes[Attribute.direction.key],
        Attribute.direction.rtl.value,
      );
    });
  ...
  group('Emoji shortcuts', () {
    test('Detects 2-characters shortcut', () {
      final document = Document.fromDelta(Delta()..insert(':)\n'));
      final performed = autoformats.run(document, 1, ')');
      expect(performed, true);
      expect(
        autoformats.selection,
    ...
    test('Detects 3-characters shortcut', () {
      final document = Document.fromDelta(Delta()..insert('o:)\n'));
    ...
    test('No trigger of detection if inserting other than space', () {
      final document = Document.fromDelta(Delta()..insert(':)p\n'));
...
class FakeAutoFormat extends AutoFormat {
  ...
  @override
  AutoFormatResult? apply(Document document, int position, String data) {
    if (data == trigger) {
      final change = Delta()
        ..retain(position)
        ..insert('Fake');
