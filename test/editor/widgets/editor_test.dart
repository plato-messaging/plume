...
void main() {
  ...
  group('$RawEditor', () {
    testWidgets('Scrolls to reveal the cursor when keyboard pops up',
      ...
      final controller =
          PlumeController(document: Document.fromDelta(delta));
      final editor = MaterialApp(
        home: Scaffold(
          body: Column(
            children: [
              Expanded(child: PlumeEditor(controller: controller)),
            ],
          ),
        ),
      );
      ...
      expect(
          tester.getRect(find.byType(PlumeEditor)).contains(
    ...
    group('Cursor offset to text position', () {
      MaterialApp widgetWithPadding(
          PlumeController controller, EdgeInsetsGeometry padding) {
        final editor = MaterialApp(
          home: Scaffold(
            body: Column(
              children: [
    ...
    group('PlumeTheme', () {
      testWidgets('allows merging attribute theme data', (tester) async {
        ...
        var editor =
            EditorSandBox(tester: tester, document: doc, plumeTheme: theme);
      ...
      Future<void> testStrutUsage(WidgetTester tester, Delta delta) async {
        ...
        var editor = EditorSandBox(
            tester: tester,
            document: doc,
            plumeTheme: PlumeThemeData.fallback(context).copyWith(
    ...
    testWidgets('ability to paste upon long press on an empty document',
      ...
      final editor = EditorSandBox(
          tester: tester, document: Document(), autofocus: true);
      await editor.pump();
    ...
    testWidgets('creating editor without focusNode does not throw _CastError',
        (tester) async {
      final widget =
          MaterialApp(home: PlumeEditor(controller: PlumeController()));
    ...
    group('Text selection', () {
      ...
      testWidgets(
        ...
        await tester.tapAt(tester.getTopRight(find.byType(PlumeEditor)) +
            const Offset(-1, 1));
      ...
      testWidgets(
        ...
        await editor.updateSelection(base: 1, extent: 1);
        await tester.sendKeyDownEvent(LogicalKeyboardKey.shift);
        await tester.tapAt(tester.getBottomRight(find.byType(PlumeEditor)) -
            const Offset(1, 1));
        await tester.sendKeyUpEvent(LogicalKeyboardKey.shift);
        await tester.pump();
        expect(
      ...
      testWidgets(
        ...
        final editor = EditorSandBox(tester: tester, autofocus: true);
        await editor.pump();
        await tester.tapAt(tester.getBottomLeft(find.byType(PlumeEditor)) +
            const Offset(10, -1));
        await tester.pump();
        expect(
            editor.selection,
    ...
    group('Spell check', () {
      ...
      testWidgets('suggests correction on initial load (Android)',
        ...
        await tester.pump();
        expect(
            find.byType(PlumeSpellCheckSuggestionsToolbar), findsOneWidget);
      }, variant: TargetPlatformVariant.only(TargetPlatform.android));
      ...
      testWidgets('suggests correction on initial load (iOS)', (tester) async {
        ...
        await tester.tapAt(tester.getTopLeft(find.byType(PlumeEditor)) +
            const Offset(1, 1));
        await tester.pump();
        expect(find.byType(PlumeCupertinoSpellCheckSuggestionsToolbar),
            findsOneWidget);
      }, variant: TargetPlatformVariant.only(TargetPlatform.iOS));
