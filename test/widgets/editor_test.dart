...
import 'package:plume/src/services/spell_check_suggestions_toolbar.dart';
import 'package:plume/src/widgets/checkbox.dart';
import 'package:plume/src/widgets/keyboard_listener.dart';
import 'package:plume/src/widgets/system_context_menu.dart';
import 'package:plume/src/widgets/text_selection.dart';
...
void main() {
  ...
  group('$RawEditor', () {
    testWidgets('Scrolls to reveal the cursor when keyboard pops up', (
      ...
      final controller = PlumeController(document: Document.fromDelta(delta));
      final editor = MaterialApp(
        home: Scaffold(
          body: Column(
            children: [Expanded(child: PlumeEditor(controller: controller))],
          ),
        ),
      );
      ...
      final renderEditor = tester
          .state<EditorState>(find.byType(RawEditor))
          .renderEditor;
      final endpoint = renderEditor.getEndpointsForSelection(
        renderEditor.selection,
      );
      expect(
        tester
            .getRect(find.byType(PlumeEditor))
            .contains(
    ...
    group('Cursor offset to text position', () {
          ...
          home: Scaffold(
            body: Column(
              children: [
                ...
                Expanded(
                  child: PlumeEditor(controller: controller, padding: padding),
                ),
              ],
            ),
          ),
      ...
      testWidgets('Cursor offset to text position', (tester) async {
        ...
        final controller = PlumeController(document: Document.fromDelta(delta));
        final editor = widgetWithPadding(
          controller,
        ...
        final renderEditor = tester
            .state<EditorState>(find.byType(RawEditor))
            .renderEditor;
        final curserCenter = renderEditor
            .getLocalRectForCaret(TextPosition(offset: 5))
            .center;
        final position = renderEditor.getPositionForOffset(
          renderEditor.localToGlobal(curserCenter),
        );
      ...
      testWidgets('Directional padding - Cursor offset to text position', (
        ...
        MaterialApp editor = widgetWithPadding(
          controller,
          EdgeInsetsDirectional.only(start: 20, end: 16),
        );
        await tester.pumpWidget(editor);
        final renderEditor = tester
            .state<EditorState>(find.byType(RawEditor))
    ...
    testWidgets('Allows children to capture events when scrolled', (
      ...
      await tester.pumpAndSettle();
      final checkBox = find.byType(PlumeCheckbox);
      ...
      expect(
        ...
        generateDelta(withBoxChecked: true),
      );
    });
    ...
    group('PlumeTheme', () {
      testWidgets('allows merging attribute theme data', (tester) async {
        var delta = Delta()
          ..insert(
            'Website',
            Attribute.link.fromString('https://github.com').toJson(),
          )
          ..insert('\n');
    ...
    testWidgets('toggle enabled state', (tester) async {
      ...
      final widget = tester.widget<PlumeField>(find.byType(PlumeField));
    ...
    testWidgets('ability to paste upon double-tap on an empty document', (
      ...
      await editor.pump();
      expect(find.text('Paste'), findsNothing);
      await tester.tap(find.byType(PlumeEditor));
      await tester.tap(find.byType(PlumeEditor));
      await tester.pump();
      final finder = find.text('Paste');
      expect(finder, findsOneWidget);
    ...
    testWidgets('creating editor without focusNode does not throw _CastError', (
      ...
      final widget = MaterialApp(
        home: PlumeEditor(controller: PlumeController()),
      );
    ...
    group('TextWidthBasis', () {
      testWidgets('shrink editor to content size in read only', (tester) async {
        ...
        final longestLine = state.renderEditor.childAtPosition(
          TextPosition(offset: 1),
        );
        expect(
          state.renderEditor.size.width.toInt(),
    ...
    testWidgets('Copy intent sends data to clipboard manager', (tester) async {
      ...
      final editor = EditorSandBox(
        ...
        autofocus: true,
        clipboardManager: PlumeCustomClipboardManager(
          getData: () => throw UnimplementedError(),
          setData: (data) async => sentData = data,
        ),
      );
      await editor.pump();
      final RawEditorState state = tester.state<RawEditorState>(
        find.byType(RawEditor),
      );
    ...
    testWidgets('Paste intent gets data from clipboard manager', (
      ...
      prepareClipboard();
      var data = PlumeClipboardData(plainText: 'Test');
      final editor = EditorSandBox(
        tester: tester,
        document: Document(),
        autofocus: true,
    ...
    testWidgets('Paste updates selection correctly', (tester) async {
      ...
      data = PlumeClipboardData(delta: Delta()..insert('Text', {'b': true}));
    ...
    group('Text selection', () {
      testWidgets(
        'disabled selection interaction disables associated gestures',
        (tester) async {
          final editor = EditorSandBox(
            tester: tester,
            enableSelectionInteraction: false,
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          await tester.tapAt(
            tester.getTopRight(find.byType(PlumeEditor)) + const Offset(-1, 1),
          );
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          await editor.pump();
          await tester.tapAt(
            tester.getBottomLeft(find.byType(PlumeEditor)) +
                const Offset(10, -1),
          );
          await tester.pump();
          expect(
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          await editor.pump();
          await tester.tapAt(
            tester.getTopLeft(find.byType(PlumeEditor)) + const Offset(30, 1),
          );
          tester.binding.scheduleWarmUpFrame();
          await tester.pump();
      ...
      testWidgets('dragging collapsed selection shows magnifier', (
        ...
        await editor.pump();
        final gesture = await tester.startGesture(
          tester.getBottomLeft(find.byType(PlumeEditor)) + const Offset(10, -1),
        );
        await gesture.moveBy(
          tester.getBottomLeft(find.byType(PlumeEditor)) + const Offset(40, 0),
      ...
      testWidgets(
        'iOS dragging selection start past end handle swaps handles',
        (tester) async {
          final document = Document.fromJson([
            {'insert': 'Some piece of text\n'},
          ]);
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          ); // Select all in other than macOS
          await tester.pump();
          expect(
            editor.selection,
            const TextSelection(
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          final document = Document.fromJson([
            {'insert': '$text\n'},
            {
              'insert': {'_type': 'something', '_inline': true},
            },
            {
              'insert': {'_type': 'something', '_inline': true},
            },
    ...
    group('didUpdateWidget', () {
      testWidgets(
        ...
        (tester) async {
          ...
          final widget = MaterialApp(
            home: TestUpdateWidget(focusNodeAfterChange: expFocus),
          );
          await tester.pumpWidget(widget);
          final initialState = tester.state<RawEditorState>(
            find.byType(RawEditor),
          );
    ...
    group('Field', () {
      testWidgets(
        'creating field without focusNode does not throw _CastError',
        (tester) async {
          final widget = MaterialApp(
            home: PlumeField(controller: PlumeController()),
          );
          await tester.pumpWidget(widget);
          // Fails if thrown
        },
      ...
      testWidgets('handle insertion embeds with no baseline', (tester) async {
        ...
        final widget = MaterialApp(home: PlumeField(controller: controller));
        await tester.pumpWidget(widget);
        await tester.tap(find.byType(PlumeField));
        await tester.pump();
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          await tester.pumpWidget(widget);
          final field = tester.widget<PlumeField>(
      ...
      testWidgets('shows cursor on screen when scrollable with scroll parent', (
        ...
        final renderEditor = tester
            .state<RawEditorState>(find.byType(RawEditor))
            .renderEditor;
        final editorRect = Rect.fromLTWH(
          0,
          0,
          renderEditor.size.width,
          renderEditor.size.height,
        );
        var caretRect = renderEditor.getLocalRectForCaret(
          TextPosition(offset: 200),
        );
