...
void main() {
  group('$EmbedRegistry', () {
    ...
    test('create with configurations', () {
      ...
      final fakeBlock = registry.blockEmbed(
        EmbeddableObject('fake_block', inline: false),
      );
      expect(fakeBlock, isA<FakeBlockEmbed>());
    });

    test('create fallback with configurations', () {
      final registry = EmbedRegistry.fallbackWithConfigurations([
        FakeBlockEmbed(),
      ]);
      final fakeBlock = registry.blockEmbed(
        EmbeddableObject('fake_block', inline: false),
    ...
    test('spanEmbed - finds a block embed', () {
      ...
      try {
        registry.spanEmbed(
          EmbeddableObject(FakeBlockEmbed().key, inline: true),
        );
    ...
    test('blockEmbed - finds a span embed', () {
      ...
      try {
        registry.blockEmbed(
          EmbeddableObject(FakeSpanEmbed().key, inline: false),
        );
      } on AssertionError catch (_) {
        return;
      }
