...
final rightAttrs = Style().merge(Attribute.right);
final ulAttrs = Style().merge(Attribute.ul);
final olAttrs = Style().merge(Attribute.ol);
final h1Attrs = Style().merge(Attribute.h1);
...
void main() {
  group('$BlockNode', () {
    ...
    test('toString', () {
      ...
      block.applyAttribute(Attribute.ul);
      block.add(line);
      final expected = '§ {ul}\n  └ ¶ ⟨London "Grammar"⟩ ⏎';
    ...
    test('format first line as list', () {
      ...
      expect(root.childCount, 1);
      final block = root.first as BlockNode;
      expect(block.style.get(Attribute.block), Attribute.ul);
    ...
    test('format first line as list and right aligned', () {
      ...
      expect(block.first, const TypeMatcher<LineNode>());
      final line = block.first as LineNode;
      expect(line.style.get(Attribute.alignment), Attribute.right);
    });
