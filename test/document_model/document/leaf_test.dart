...
final boldUnsetStyle = Style().put(Attribute.bold.unset);
final italicStyle = Style().merge(Attribute.italic);
...
void main() {
  group('TextNode', () {
    ...
    test('toString', () {
      node.applyAttribute(Attribute.bold);
      node.applyAttribute(Attribute.link.fromString('link'));
    ...
    test('format full segment', () {
      ...
      expect(node.value, 'London "Grammar"');
      expect(node.style.values, [Attribute.bold]);
    });
  ...
  group('LeafNode', () {
    ...
    test('applyStyle allows inline styles only', () {
      final text = LeafNode('Text');
      final style = Style().put(Attribute.block.numberList);
