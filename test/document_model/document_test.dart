  ...
  group('$Document', () {
    test('validates for doc delta', () {
      var badDelta = Delta()
        ...
        ..insert('\n');
      expect(() {
        Document.fromJson(jsonDecode(jsonEncode(badDelta)));
      }, throwsArgumentError);
    });
    ...
    test('json serialization', () {
      ...
      final doc = Document.fromJson(json.decode(jsonData) as List);
    ...
    test('load document with 0.x version embeds', () {
      final delta = Delta()
        ...
        ..insert('\n')
        ..insert('Los Angeles\n');
      final doc = Document.fromJson(jsonDecode(jsonEncode(delta)));
    ...
    test('document delta must end with line-break character', () {
      ...
      expect(() {
        Document.fromJson(jsonDecode(jsonEncode(delta)));
      }, throwsA(const TypeMatcher<AssertionError>()));
    });

    test('lookupLine', () {
      final doc = dartconfDoc();
      doc.format(20, 1, Attribute.bq);
    ...
    test('format ignores empty changes', () async {
      ...
      var change = doc.format(1, 0, Attribute.bold);
      doc.close();
    ...
    test('format returns actual change delta', () {
      final doc = dartconfDoc();
      final change = doc.format(0, 15, Attribute.ul);
    ...
    test('replace applies heuristic rules', () {
      final doc = dartconfDoc();
      doc.format(0, 0, Attribute.h1);
      doc.replace(8, 1, ' ');
      expect(doc.root.children, hasLength(1));
      final line = doc.root.children.first as LineNode;
      expect(line.style.get(Attribute.heading), Attribute.h1);
