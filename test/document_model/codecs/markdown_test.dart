...
import 'package:plume/document_model/codecs.dart';
...
void main() {
  group('ParchmentMarkdownCodec.decode', () {
    ...
    test('ul', () {
      ...
      final act = markdownCodec.decode(markdown).toDelta();
      final exp = Delta()
    ...
    test('multiple styles', () {
      final delta = markdownCodec.decode(markdown);
      final andBack = markdownCodec.encode(delta);
      expect(andBack, markdown);
    });
  });
  ...
  group('ParchmentMarkdownCodec.encode', () {
    test('split adjacent paragraphs', () {
      ...
      final result = markdownCodec.encode(Document.fromDelta(delta));
      expect(result, 'First line\n\nSecond line\n\n');
    });

    test('bold italic strike though', () {
      void runFor(Attribute<bool> attribute, String expected) {
        final delta = Delta()
          ..insert('This ')
    ...
    test('intersecting inline styles', () {
      ...
      final i = Attribute.italic.toJson();
    ...
    test('links', () {
      final b = Attribute.bold.toJson();
      final link = Attribute.link.fromString('https://github.com');
      final delta = Delta()
        ..insert('This')
        ..insert(' house ', b)
        ..insert('is a')
    ...
    test('heading styles', () {
      ...
      runFor(Attribute.h1, 'Title', '# Title\n\n');
      runFor(Attribute.h2, 'Title', '## Title\n\n');
      runFor(Attribute.h3, 'Title', '### Title\n\n');
      runFor(Attribute.h4, 'Title', '#### Title\n\n');
      runFor(Attribute.h5, 'Title', '##### Title\n\n');
      runFor(Attribute.h6, 'Title', '###### Title\n\n');
    });
    ...
    test('block styles', () {
      ...
      runFor(Attribute.ul, 'List item', '* List item\n\n');
      runFor(Attribute.ol, 'List item', '1. List item\n\n');
      runFor(Attribute.bq, 'List item', '> List item\n\n');
      runFor(Attribute.code, 'List item', '```\nList item\n```\n\n');
    });
    ...
    test('multiline blocks', () {
      ...
      runFor(Attribute.ul, 'text', '* text\n* text\n\n');
      runFor(Attribute.ol, 'text', '1. text\n2. text\n\n');
      runFor(Attribute.bq, 'text', '> text\n> text\n\n');
      runFor(Attribute.code, 'text', '```\ntext\ntext\n```\n\n');
    });
...
Plume is an **early preview** open source library.
...
Plume’s rich text editor is built with _simplicity and flexibility_ in mind. It provides clean interface for distraction-free editing. Think `Medium.com`-like experience.
