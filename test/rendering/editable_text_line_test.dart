...
import 'package:plume/src/rendering/editable_text_line.dart';
import 'package:plume/src/rendering/paragraph_proxy.dart';
...
void main() {
  ...
  group('$RenderEditableTextLine', () {
    ...
    test('Background color', () {
      final lineNode = LineNode()
        ..insert(0, 'some text', Style.fromJson({'bg': 0xffff0000}));
      final rootNode = RootNode();
      rootNode.addFirst(lineNode);
      final paintingContext = MockPaintingContext();
      final renderParagraph = RenderParagraph(
        const TextSpan(text: 'some text'),
        textDirection: TextDirection.ltr,
      );
      final renderBox = RenderEditableTextLine(
        node: lineNode,
    ...
    test('inline code', () {
      final lineNode = LineNode()
        ..insert(0, 'some text', Style.fromJson({'c': true}));
      final rootNode = RootNode();
      rootNode.addFirst(lineNode);
      final paintingContext = MockPaintingContext();
      final renderParagraph = RenderParagraph(
      ...
      expect(paintingContext.canvas.drawnRectPaint!.style, PaintingStyle.fill);
      expect(
        paintingContext.canvas.drawnRectPaint!.color.value32Bits,
